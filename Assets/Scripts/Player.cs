...
public class Player : Sound
{
    ...
    private IEnumerator ApplyStart()
    {
    ...
    private void ApplySaveBuffs()
    {
        bonusForce += upgradeCount;
        bonusDamage += upgradeCount;
        bonusHealth += upgradeCount;
        bonusSpeed += upgradeCount;
    ...
    { 
        ...
        // ѕрименение бонусного здоровь€

        //Debug.Log("¬ы вызвали делегат");
    }
    ...
    private void Update()
    {
        ...
        //string message = $"“ип:{bonusFish.GetType().Name}";
        //Debug.Log(message);
    ...
    private void Movement()
    {
        ...
        isJumping = isJumping && !groundDetection.isGround;//мы провер€ем если мы сейчас в прыжке и при этом мы не касаемс€ земли, то мы сохран€ем значение перем.isJumping, если мы падаем при этом не касаемс€ земли тогда мы попадем в if выше и начнетс€ анимаци€ падени€ мину€ startjump
        ...
        direction = new Vector2(movejoystick*(speed+bonusSpeed),rigidbd.velocity.y);
        //direction *= speed;
        //direction.y = rigidbd.velocity.y;//чтобы состовл€юща€ по оси у не стала равна 0
        if(!isBlockMovement)
